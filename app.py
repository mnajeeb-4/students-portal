...
# =====================================================================
# 2. SAFE PDF GENERATION (Only if reportlab is installed)
# =====================================================================
def generate_pdf_report(roll, name, cls, marks_dict, date_today):
    ...
    width, height = letter
    # (PDF logic remains same as before...)
...
# ---------- PAGE 2: ADMIN ----------
    ...
    if not st.session_state.admin_auth:
        ...
        with tab2:
            ...
            if data:
                if not PANDAS_AVAILABLE:
                    st.error("⚠️ Pandas library missing. Can't display database. Run `pip install pandas`")
                else:
                    df_data = []
        ...
        with tab3:
            if data:
                if not PANDAS_AVAILABLE:
                    st.error("⚠️ Pandas library missing. Can't export CSV. Run `pip install pandas`")
                else:
                    export_list = []
                    for r, info in data.items():
...
# ---------- PAGE 3: STUDENT (CRASH PROOF + BEAUTIFUL CANVA CARD) ----------
    ...
    with st.container():
        ...
        if st.button("🔍 Generate Report"):
            if roll in data:
                ...
                # =====================================================================
                # 1. PREMIUM HTML CANVA CARD (100% NO RAW TEXT ERROR)
                # =====================================================================
                html_content = f"""
                ...
                # ---------- 2. DATA TABLE & CHART (Guard against missing Pandas) ----------
                if not PANDAS_AVAILABLE:
                    st.warning("⚠️ Pandas library not installed. Data Table & Radar chart disabled. Run: `pip install pandas`")
                else:
                ...
                # ---------- 3. PDF GENERATION (Guard against missing ReportLab) ----------
                if REPORTLAB_AVAILABLE:
                    ...
                    st.warning("⚠️ ReportLab is not installed. PDF generation disabled. Please run: `pip install reportlab`")
